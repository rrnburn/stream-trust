...
import { Radio, ChevronDown, ChevronRight, Search, BookOpen, List } from 'lucide-react';
  ...
  return (
    <AppLayout>
      <div className="flex flex-col h-[calc(100vh-0px)]">
        {/* Toggle between channel list and guide */}
        <div className="flex items-center gap-2 px-4 pt-3 pb-1">
          <Button
            variant={showGuide ? 'outline' : 'default'}
            size="sm"
            onClick={() => setShowGuide(false)}
            className="gap-1.5 text-xs"
          >
            <List className="w-3.5 h-3.5" /> Channels
          </Button>
          <Button
            ...
            className="gap-1.5 text-xs"
          >
            <BookOpen className="w-3.5 h-3.5" /> TV Guide
          </Button>
        </div>
        ...
        {showGuide ? (
          <div className="flex-1 p-4 overflow-auto">
            <TVGuide
              ...
              loading={false}
              onChannelSelect={(ch) => {
                const found = channels.find(c => c.id === ch.id);
                if (found) {
